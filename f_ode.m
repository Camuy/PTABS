function dxdt = f_ode(A, B, Fgal, y, time, dumper)
...
% x = @(x1)@(x2) [x1; x2];
% z = y - x;
% f1 = @(h) interp1(F.z, F.normal(1, :), z);
% f2 = @(h) interp1(F.z, F.normal(2, :), z);
% f3 = 0;
% f4 = 0;

%dxdt =  @(t, x) A\([interp1(Fgal.z, Fgal.normal(1, :), interp1(time, y, t) - x(3)); interp1(Fgal.z, Fgal.normal(2, :), interp1(time, y, t) - x(4)); 0; 0] - B*x);
...
dxdt =  @(t, x) A\( ...
    ...
    interp1(Fgal.z, Fgal.normal(2, :), interp1(time, y, t) - x(4)) + interp1(dumper.z, dumper.k1, x(4) - x(3) + dumper.l + dumper.x1)*(dumper.x1 + dumper.l) - interp1(dumper.z, dumper.k2, -(x(4) - x(3) + dumper.l + dumper.x2))*(dumper.x2 - dumper.l); ...
    0; ...
